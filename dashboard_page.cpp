...
static void refresh_heat_stats() {
    ...
    sprintf(m,
        "<span size='11000' color='#505050'>累计阅读</span>\n"
        "<span size='14000' weight='bold'>%.1f 小时</span>\n"
        "<span size='11000' color='#505050'>活跃天数</span>\n"
        "<span size='14000' weight='bold'>%d 天</span>\n"
        "<span size='11000' color='#505050'>最长连读</span>\n"
        "<span size='14000' weight='bold'>%d 天</span>\n"
        "<span size='11000' color='#505050'>日均阅读</span>\n"
        "<span size='14000' weight='bold'>%d 分钟</span>",
...
GtkWidget* create_dashboard_page() {
    ...
    gtk_label_set_markup(GTK_LABEL(heat_title_lbl), "<span size='13000' weight='bold'>月度阅读热力图</span>");
    ...
    // Year/Month controls row: [<- 年] [年] [年 ->] [<- 月] [月] [月 ->]
    GtkWidget *ctrl_hbox = gtk_hbox_new(FALSE, 2);
    gtk_misc_set_alignment(GTK_MISC(ctrl_hbox), 0.5, 0.5);
    ...
    gtk_widget_set_size_request(g_heat_year_lbl, 80, -1);
    gtk_misc_set_alignment(GTK_MISC(g_heat_year_lbl), 0.5, 0.5);
    gtk_box_pack_start(GTK_BOX(ctrl_hbox), g_heat_year_lbl, FALSE, FALSE, 0);
    ...
    gtk_box_pack_start(GTK_BOX(ctrl_hbox), g_heat_month_lbl, FALSE, FALSE, 0);
    ...
    // Heatmap drawing area (smaller - ~140px)
    g_heat_drawing_area = gtk_drawing_area_new();
    gtk_widget_set_size_request(g_heat_drawing_area, -1, 140);
    ...
    // Right: compact stats text (4 items, tightly packed)
